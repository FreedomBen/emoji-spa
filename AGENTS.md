...
- `frontend/` — Source for the static frontend (HTML/CSS/JS) loaded by Tauri, including `app.js` and emoji metadata.
- `dist/` — Generated frontend bundle emitted by Vite.
...
- `make dev` — Build the Vite bundle, then run `cargo tauri dev`.
- `make update-emoji` — Regenerate `frontend/public/emoji-cldr.json` from the latest Unicode data.
...
- JavaScript in `frontend/`: keep 2-space indentation, `const`/`let` usage, and naming consistent with existing code in `app.js`.
...
- For frontend logic, follow the high-level scenarios in `TEST_SUITE_PLAN.md` when introducing a JS test setup (e.g., browser-like unit tests around `frontend/app.js`).
...
- When running `make update-emoji`, verify the generated `frontend/public/emoji-cldr.json` is valid JSON before committing.
